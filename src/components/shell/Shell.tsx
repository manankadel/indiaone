    ...
    <header className="sticky top-0 z-40 backdrop-blur bg-[#FFFBF5]/80 border-b border-zinc-200">
      <div className="mx-auto max-w-[1160px] px-4 sm:px-6 h-[64px] flex items-center justify-between gap-2">
        ...
        <nav className="hidden md:flex items-center gap-1">
          ...
          <div className="ml-2 flex items-center gap-1 rounded-full border border-zinc-200 bg-white p-1">
            <button onClick={()=>setLocale("en")} className={`rounded-full px-3 py-1 text-xs font-semibold ${locale==="en" ? "bg-zinc-900 text-white" : "text-zinc-600"}`}>EN</button>
            <button onClick={()=>setLocale("hi")} className={`rounded-full px-3 py-1 text-xs font-semibold ${locale==="hi" ? "bg-zinc-900 text-white" : "text-zinc-600"}`}>हि</button>
          </div>
          <Link href="/#start" className="ml-1 rounded-full bg-[#FF5A1F] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#E8541B]">Start Fraud First Aid</Link>
        </nav>
        <div className="flex items-center gap-2 md:hidden">
          <div className="flex items-center rounded-full border border-zinc-200 bg-white p-1">
            <button onClick={()=>setLocale("en")} className={`rounded-full px-2.5 py-1 text-xs font-semibold ${locale==="en" ? "bg-zinc-900 text-white" : ""}`}>EN</button>
            <button onClick={()=>setLocale("hi")} className={`rounded-full px-2.5 py-1 text-xs font-semibold ${locale==="hi" ? "bg-zinc-900 text-white" : ""}`}>हि</button>
          </div>
      ...
      {open && (
        <div className="md:hidden border-t border-zinc-200 bg-white px-4 py-4 space-y-2">
          ...
          <p className="text-xs text-zinc-500 pt-2">Mock creds: <span className="font-mono">citizen@indiaone.demo / demo1234</span> · Voice: Chrome/Edge + Android</p>
        </div>
      )}
    </header>
  );
        ...
        <footer className="mt-16 border-t border-zinc-200 bg-white">
          <div className="mx-auto max-w-[1160px] px-4 sm:px-6 py-10 grid md:grid-cols-3 gap-8 text-sm">
            ...
            <div className="text-zinc-600">
              ...
              <div className="mt-2">Mock creds: citizen@indiaone.demo / demo1234 · All data synthetic. Voice is enhancement, text is baseline.</div>
            </div>
          </div>
        </footer>
      </StoreProvider>
